...
if __name__ == "__main__":
    ...
    # Сначала попробуем получить через Tinkoff
    try:
        tinkoff_client = TinkoffApiClient(ticker)
        price = tinkoff_client.get_current_price()
        print(f"Tinkoff price for {ticker}:", json.dumps(price, indent=2))
        
        info = tinkoff_client.get_instrument_info()
        print(f"Tinkoff info for {ticker}:", json.dumps(info, indent=2))
        
        daily = tinkoff_client.get_daily_data(days=5)
        print(f"Tinkoff daily data for {ticker} (5 days):", json.dumps(daily, indent=2))
    except Exception as e:
        print(f"Error with Tinkoff API: {str(e)}")
    
    print("\n" + "-"*50 + "\n")
    
    # Затем через Alpha Vantage, с перестраховкой на случай если не найдет просто OZON
    try:
        # Попробуем сначала просто OZON
        alpha_client = AlphaVantageApiClient(ticker)
        price = alpha_client.get_current_price()
        
        # Если не найдено, попробуем добавить суффикс биржи
        if "error" in price:
            print(f"Alpha Vantage: ticker {ticker} not found, trying {ticker}.US")
            alpha_client = AlphaVantageApiClient(f"{ticker}.US")
            price = alpha_client.get_current_price()
    ...
    # Дополнительно попробуем OZON.US для Alpha Vantage
    try:
        us_ticker = f"{ticker}.US"
        alpha_client_us = AlphaVantageApiClient(us_ticker)
        price = alpha_client_us.get_current_price()
        if "error" not in price:
            print(f"\nAlpha Vantage price for {us_ticker}:", json.dumps(price, indent=2))
    except Exception as e:
        print(f"Error with Alpha Vantage API for {us_ticker}: {str(e)}")